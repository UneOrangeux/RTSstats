(function(){
  ...
  const state = { rows: [] };
  const logEl = $("#log");
  function log(){ const msg = Array.from(arguments).join(" "); logEl.textContent += msg + "\n"; console.log.apply(console, arguments); }
  ...
  async function build(){
    try{
      ...
      const base = ($("#apiBase")?.value||"https://v1.api.tycoon.community").trim();
      const path = ($("#streakPath")?.value||"/main/streak/{vrpId}").trim();
      const serversJson = ($("#serversJson")?.value||"https://cdn.tycoon.community/servers.json").trim();
  ...
  document.getElementById("testBtn").addEventListener("click", ()=> setStatus("Button works ✔"));
